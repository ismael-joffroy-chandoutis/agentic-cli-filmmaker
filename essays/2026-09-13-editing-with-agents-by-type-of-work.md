...
## Update, 14 September: what a night of real tests changed

I installed and ran eleven canvases and agent-first editors under one protocol (three images and one video placed, MCP session opened and counted, a generation on a local ComfyUI, an export), plus two editors against a three-clip cut with six splits, a title, a track change and an FCPXML round trip into Resolve. What held up:

- **Palmier** passed the round trip: every cut landed on the exact frame in Resolve, media relinked by itself, title and moved clip on the upper track. It is the tool for agent-driven excerpts with a Final Cut return. Still closed since 0.8.1, macOS 26 only, and it writes FCPXML 1.10.
- **SpliceKit works on Final Cut 12.3** once you apply the two open pull requests that repair the from-source build (they add stub functions for the Blackmagic RAW SDK; applying both at once duplicates them, remove one copy) and run the patch script yourself. The JSON-RPC server answers in three seconds, 222 MCP tools, reads in a few milliseconds, no crash at launch. It cannot create a project or place a clip with source bounds by parameters (those go through Final Cut's own dialogs), so it is an analysis and finishing layer on an open library, transcription, silences, captions, direct FCPXML export, rather than a way to build a cut. Every Apple update breaks it again.
- **basketikun/infinite-canvas** is the best canvas and the best canvas MCP I tested (34 typed operations against the app's own store, video plays on the card, a generation lands next to its source). It has no notion of a take, no timeline, no export to an editor, and ComfyUI is not a native backend. **BeatDesign** has the best agent-driven timeline (29 tools with expected revision and receipts) and a single generation provider. Jaaz is asleep, Raccord generates only through a cloud key, NodeTool is a storyboard tool that takes the foreground on launch.
- My own shot store's board was the only chain that took a 151 MB ProRes, generated on the local rig and linked the take to its scene. It also had the two worst bugs of the night: 10-bit HEVC proxies render black in Chromium, and the shipped ComfyUI workflows were placeholders. Keep it, fix those two first, borrow the gestures above.
- The abandoned OpenTimelineIO FCPXML adapter reads Palmier's 1.10 file but rewrites it as 1.8 with invented media paths, drops the title and the audio, and Resolve refuses the result. OpenTimelineIO stays the exchange format for structure; the FCPXML has to be written by something maintained.
- Deforum is frozen on A1111 1.9 and still alive as an instrument: no 2026 video model reproduces its recursive img2img loop with depth reprojection and per-frame schedules. Keep the old environment, qualify the official ComfyUI port.

Two lessons for anyone letting agents run installs overnight on a Mac: XProtect flags any unsigned Electron binary launched from a node_modules folder as malware and trashes it, three full-screen alerts in my case; and an agent that activates an application steals the keyboard from whoever is typing. Background launches only, screenshots by window id.
